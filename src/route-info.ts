...
export class RouteInfo {
    ...
    private readonly _routeParams = [] as Array<RouteParam>;
    private readonly _routeParamsRegistry: Record<string, RouteParam | null> =
        {};
    private readonly _reactRoute: string | null = null;
    private readonly _pathSegments = [] as Array<string>;
    ...
    public get reactRoute(): string {
        given(this, "this").ensure(
            (t) => t._reactRoute != null,
            "not available in UrlGenerator mode",
        );
        return this._reactRoute!;
    }
    public get params(): ReadonlyArray<RouteParam> {
        return this._routeParams;
    }
    public get pathSegments(): ReadonlyArray<string> {
    ...
    public constructor(routeTemplate: string, isUrlGenerator = false) {
        // true if used purely for url generation (only by utils)
        ...
        if (!isUrlGenerator) {
            ...
            if (routeTemplate.length > 1 && routeTemplate.endsWith("/"))
                routeTemplate = routeTemplate.substr(
                    0,
                    routeTemplate.length - 1,
                );
        }
        ...
        if (!isUrlGenerator) {
            this._reactRoute = this._generateVueRoute(this._routeTemplate);
        ...
        for (const key in values) {
            ...
            if (routeParam) {
                ...
                if (val == null && routeParam.isQuery && routeParam.isOptional)
                    // only query params can be optional anyway
                    replacement = "";
    ...
    private _populateRouteParams(): void {
        let index = 1;
        for (const routeParam of this._extractTemplateParams(
            this._routeTemplate,
        ).map((t) => new RouteParam(t))) {
            const key = routeParam.paramKey.toLowerCase();
            if (this._routeParamsRegistry[key])
                throw new ApplicationException(
                    "Invalid route template. Duplicate route params (case insensitive) detected.",
                );
    ...
    private _extractTemplateParams(routeTemplate: string): Array<string> {
        const templateParams: Array<string> = [];
        ...
        for (let i = 0; i < routeTemplate.length; i++) {
            if (routeTemplate[i] === "?" && !startFound) {
                if (queryFound)
                    throw new ApplicationException(
                        "Invalid route template. Unresolvable '?' characters detected.",
                    );
            ...
            if (routeTemplate[i] === "{") {
                if (startFound)
                    throw new ApplicationException(
                        "Invalid route template. Braces do not match.",
                    );
    ...
    private _populatePathSegments(): void {
        const routeTemplate = this._reactRoute!;
        const pathSegments: Array<string> = [];

        pathSegments.push("/");

        for (const item of routeTemplate.split("/")) {
            if (item.isEmptyOrWhiteSpace() || item.startsWith(":")) continue;

            if (pathSegments.some((t) => t === item))
                throw new ArgumentException(
                    "routeTemplate",
                    "cannot contain duplicate segments",
                );
