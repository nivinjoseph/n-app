...
import type { PageRegistration } from "./page-registration.js";
...
export class PageTreeBuilder {
    ...
    public constructor(
        root: Page,
        pageRegistrations: ReadonlyArray<PageRegistration>,
    ) {
        given(root, "root").ensureHasValue().ensureIsType(Page);
        given(pageRegistrations, "pageRegistrations")
            .ensureHasValue()
            .ensureIsArray()
    ...
    private _buildTree(): void {
        for (const registration of this._registrations) {
            ...
            for (const segment of registration.route.pathSegments) {
                if (segment === currentNode.segment) continue;

                let node = currentNode.children.find(
                    (t) => t.segment === segment,
                );
                if (!node) node = new Page(segment, currentNode);
    ...
    private _shakeTree(): ReadonlyArray<Page> {
        const nodesToProcess: Array<Page> = [];
        // skip root
        ...
        if (this._root.registration) return [this._root];

        const children = this._root.children;
        for (const child of children) child.changeParent(null);
