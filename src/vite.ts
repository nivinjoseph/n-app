...
export function clientTestPlugins(): Array<Plugin> {
    return [
        swc.vite({
            include: /\.tsx?$/,
            jsc: {
...
export function stubModulesInDev(specifiers: ReadonlyArray<string>): Plugin {
    ...
    return {
        ...
        load(id): string | null {
            ...
            const message =
                `"${specifier}" is stubbed out of the Vite dev client bundle by ` +
