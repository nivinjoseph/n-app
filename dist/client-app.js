...
export class ClientApp {
    ...
    _initialRoute = null;
    _unknownRoute = null;
    _routeErrorFallbackComponent = null;
    _startupScript = null;
    ...
    constructor(appElementSelector) {
        given(appElementSelector, "appElementSelector")
            .ensureHasValue()
            .ensureIsString()
            .ensure((t) => t.startsWith("#"), "ClientApp appElementSelector must start with '#'");
    ...
    registerInstaller(installer) {
        ...
        given(this, "this").ensure((t) => !t._isBootstrapped, ALREADY_BOOTSTRAPPED);
    ...
    registerPages(...pages) {
        given(pages, "pages")
            ...
            .ensureIsNotEmpty();
        given(this, "this").ensure((t) => !t._isBootstrapped, ALREADY_BOOTSTRAPPED);
        this._pageRegistrations.push(...pages.map((t) => new PageRegistration(t.viewModel, t.view)));
        return this;
    }
    /**
     ...
     */
    discoverPages(viewModelModules, viewModules) {
        given(viewModelModules, "viewModelModules")
            .ensureHasValue()
            .ensureIsObject();
        given(viewModules, "viewModules").ensureHasValue().ensureIsObject();
        given(this, "this").ensure((t) => !t._isBootstrapped, ALREADY_BOOTSTRAPPED);
        this._pageRegistrations.push(...discoverPageDetails(viewModelModules, viewModules).map((t) => new PageRegistration(t.viewModel, t.view)));
        return this;
    }
    registerComponents(...components) {
        given(components, "components")
            ...
            .ensureIsNotEmpty();
        given(this, "this").ensure((t) => !t._isBootstrapped, ALREADY_BOOTSTRAPPED);
        this._componentRegistrations.push(...components.map((t) => new ComponentRegistration(t.viewModel)));
        return this;
    }
    /**
     ...
     */
    discoverComponents(modules) {
        given(modules, "modules").ensureHasValue().ensureIsObject();
        given(this, "this").ensure((t) => !t._isBootstrapped, ALREADY_BOOTSTRAPPED);
        this._componentRegistrations.push(...discoverComponentViewModels(modules).map((t) => new ComponentRegistration(t)));
        return this;
    }
    configureInitialRoute(route) {
        given(route, "route").ensureHasValue().ensureIsString();
        given(this, "this").ensure((t) => !t._isBootstrapped, ALREADY_BOOTSTRAPPED);
        this._initialRoute = route.trim();
        return this;
    }
    configureUnknownRoute(route) {
        given(route, "route").ensureHasValue().ensureIsString();
        given(this, "this").ensure((t) => !t._isBootstrapped, ALREADY_BOOTSTRAPPED);
        this._unknownRoute = route.trim();
        return this;
    }
    configureRouteErrorFallbackComponent(element) {
    ...
    _registerViewModels() {
        ...
        for (const reg of this._pageRegistrations) {
            if (routeKeys.has(reg.route.routeKey)) {
                throw new ApplicationException(`Route conflict detected for Page registration with name '${reg.name}'`);
            }
    ...
    async _bootstrapContainer() {
        await this._container.bootstrap();
        this._rootScope = this._container;
    }
    _mount() {
        const rootElement = document.querySelector(this._appElementSelector);
        if (rootElement === null)
            throw new Error(`ClientApp: element '${this._appElementSelector}' not found in document.`);
        if (this._rootScope == null)
            throw new Error("ClientApp: rootScope missing at mount.");
        createRoot(rootElement).render(_jsx(StrictMode, { children: _jsx(RootContext.Provider, { value: this._rootScope, children: _jsx(RouterProvider, { router: this._createRouting() }) }) }));
    }
    _createRouting() {
        ...
        if (this._initialRoute)
            reactRouterRoutes.push({
                path: "/",
                element: _jsx(Navigate, { to: this._initialRoute, replace: true }),
            });
        reactRouterRoutes.push(...pageTree.map((t) => t.createReactRouterRoute(this._routeErrorFallbackComponent)));
        if (this._unknownRoute)
            reactRouterRoutes.push({
                path: "*",
                element: _jsx(Navigate, { to: this._unknownRoute, replace: true }),
            });
