...
export function useComponentViewModel(componentViewModelType, ...props) {
    const pageCtx = usePageContext();
    const propsZero = props[0];
    const [result] = useState(() => {
        const vm = pageCtx.resolve(Utils.getTypeName(componentViewModelType));
        autoMakeObservable(vm);
        vm._$startObserving();
        if (propsZero !== undefined)
            vm._$applyProps(propsZero);
        return { vm };
    });
    ...
    useEffect(() => {
        if (propsZero === undefined)
            return;
        const changed = result.vm._$applyProps(propsZero);
        if (!changed)
            return;
        void (async () => {
            try {
                await result.vm.onPropsChanged();
            }
            catch (err) {
...
//# sourceMappingURL=use-component-view-model.js.map